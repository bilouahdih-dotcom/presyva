...
Site vitrine + page de vente pour la collection de guides AvisReset.
...
Le guide lui-même (source + PDF) est dans `Documents\ebook-avis\`.

## À faire avant la mise en ligne

1. **Brancher le paiement.** Dans `index.html`, deux liens portent l'attribut `data-checkout` et `href="#"`.
   Remplacer par l'URL de checkout Lemon Squeezy. Tant que `href="#"`, un clic affiche une alerte de rappel.
...
4. **Adresse e-mail.** `contact@avisreset.com` est un placeholder — présent dans le footer et les pages légales.
...
Après toute modification de `Documents\ebook-avis\ebook.html` :

```bash
"/c/Program Files/Google/Chrome/Application/chrome.exe" --headless=new --disable-gpu --no-pdf-header-footer --print-to-pdf="C:\Users\bilel\Documents\ebook-avis\AvisReset-Guide-01-Gerer-les-avis-negatifs.pdf" "file:///C:/Users/bilel/Documents/ebook-avis/ebook.html"
```
...
Version sombre, calquée sur les codes des sites de studio (référence : alche.studio), avec la palette AvisReset.

**Les règles à ne pas casser si vous modifiez le site :**

| Élément | Règle |
|---|---|
| Fond | `#05090f` quasi noir. Le blanc n'est utilisé que pour le texte. |
| Accents | Bleu `#2563EB` / turquoise `#18C7C8`, **uniquement** en dégradé de CTA, en soulignés et en points d'état. Jamais en aplat de fond. |
| Titres | Manrope **weight 500** (pas 800), `line-height: 1`, `letter-spacing: -.035em`. C'est ce qui donne le côté éditorial plutôt que « SaaS ». |
...
| **Dot Field** | `#dots`, dans le hero | Grille de points qui grossissent, s'éclairent en turquoise et fuient le curseur dans un rayon de 150 px. |
...
- Le ton évite toute promesse de suppression garantie, conformément à la charte. La FAQ traite la question
  frontalement dès la première réponse : c'est un argument de crédibilité, pas une faiblesse.
...
1. Dupliquer `Documents\ebook-avis\ebook.html`, changer le numéro de guide et le titre de couverture.
2. Dans `index.html`, section « La collection » : passer la carte concernée en `class="col-card live"`
   et remplacer le tag « En préparation » par « Disponible ».
